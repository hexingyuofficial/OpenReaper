...
describe("Layer 4D call_template runtime binding", () => {
  ...
  it("keeps discovery compact and does not add a sixth MCP tool", () => {
    ...
    assert.equal(runtimeMenu.items.length, 10);
    assert.equal(runtimeMenu.items.some((item) => item.id === "macro.project.query"), true);
    assert.equal(runtimeMenu.items.some((item) => item.id === "macro.project.inspect"), true);
    ...
    assert.deepEqual(
      runtimeMenu.product_surface.startup_readiness,
      ALPHA3_BLOCK2_STARTUP_READINESS_DISCOVERY_SUMMARY,
    );
    assert.deepEqual(
    ...
    assert.equal(runtimeMenu.product_surface.startup_readiness.tool_surface.added_tools, 0);

...
const EXTRACTED_C3A_PROJECT_FILE_READ_HANDLERS = Object.freeze(new Map([
  ["template.project.read_current_project_path", ["project/read_file_state.lua", "read_current_project_path"]],
  ["template.project.read_dirty_state", ["project/read_file_state.lua", "read_dirty_state"]],
]));
const EXTRACTED_C3BC_PROJECT_FILE_SAVE_HANDLERS = Object.freeze(new Map([
  ["template.project.save_current_project", ["project/save_project_file.lua", "save_current_project"]],
  ["template.project.save_project_as", ["project/save_project_file.lua", "save_project_as"]],
]));
...
const EXTRACTED_E3_MEDIA_HANDLERS = Object.freeze(new Map([
  ...
  ["template.media.import_files_batch", ["media/e3_media_route.lua", "import_files_batch"]],
...
const EXTRACTED_E5_ROUTING_AUTOMATION_EXTRA_HANDLERS = Object.freeze(new Map([
  ...
  ["template.automation.delete_automation_item", ["routing/e5_r1_routing_read_route.lua", "delete_automation_item"]],
  ["template.automation.resolve_send_envelope", ["routing/e5_r1_routing_read_route.lua", "resolve_send_envelope"]],
  ["template.automation.ensure_fx_parameter_envelope", ["routing/e5_r1_routing_read_route.lua", "ensure_fx_parameter_envelope"]],
  ...
  ["template.automation.insert_fx_parameter_envelope_points_batch", ["routing/e5_r1_routing_read_route.lua", "insert_fx_parameter_envelope_points_batch"]],
...
const EXTRACTED_D12_TRANSPORT_SAFE_HANDLERS = Object.freeze(new Map([
  ...
  ["template.transport.set_playback_rate", ["transport/d12_transport_safe_route.lua", "d12_transport_set_playback_rate"]],
  ["template.transport.start_recording", ["transport/d12_transport_safe_route.lua", "d12_transport_start_recording"]],
  ["template.transport.stop_recording", ["transport/d12_transport_safe_route.lua", "d12_transport_stop_recording"]],
  ["template.transport.set_record_mode", ["transport/d12_transport_safe_route.lua", "d12_transport_set_record_mode"]],
  ["template.transport.set_punch_record_range", ["transport/d12_transport_safe_route.lua", "d12_transport_set_punch_record_range"]],
  ["template.transport.schedule_recording", ["transport/d12_transport_safe_route.lua", "d12_transport_schedule_recording"]],
]));
const EXTRACTED_D13_ITEMS_CORE_HANDLERS = Object.freeze(new Map([
  ...
  ["template.items.set_exact_selection", ["items/d13_items_core_route.lua", "d13_items_set_exact_selection"]],
  ...
  ["template.items.set_item_take_controls_batch", ["items/d13_items_core_route.lua", "d13_items_set_item_take_controls_batch"]],
  ...
  ["template.items.set_active_take", ["items/set_active_take.lua", "set_active_take"]],
...
const EXTRACTED_E2_FX_B1_WRITE_HANDLERS = Object.freeze(new Map([
  ...
  ["template.fx.set_parameter_assignments_batch", ["fx/e2_fx_l1_read_route.lua", "e2_fx_parameter_assignments_batch"]],
  ["template.fx.set_reaeq_bands", ["fx/e2_fx_l1_read_route.lua", "e2_fx_set_reaeq_bands"]],
...
const EXTRACTED_FIRST_REAL_A_HANDLERS = Object.freeze(new Map([
  ...
  ["template.project.create_project_map_snapshot", ["project/create_cleanup_report.lua", "create_project_map_snapshot"]],
  ["template.project.create_observation_bundle", ["project/create_cleanup_report.lua", "create_observation_bundle"]],
...
const EXTRACTED_D27_ANALYSIS_AUDIO_HANDLERS = Object.freeze(new Map([
  ["template.analysis.analyze_items_batch", ["analysis/d27_item_audio_analysis.lua", "analyze_items_batch"]],
...
const EXTRACTED_D28_SMALL_HANDLERS = Object.freeze(new Map([
  ["template.items.set_reverse", ["items/d13_items_core_route.lua", "d13_items_set_reverse"]],
  ["template.project.set_snap", ["project/d20_project_grid_snap.lua", "d20_project_set_snap"]],
...
const EXTRACTED_ALPHA3_3_LIFECYCLE_ATOM_HANDLERS = Object.freeze(new Map([
  ...
  ["template.items.glue_item", ["items/glue_item.lua", "alpha33_glue_item"]],
...
const EXTRACTED_HANDLER_ROWS = Object.freeze(new Map([
  ...
  ...EXTRACTED_C3A_PROJECT_FILE_READ_HANDLERS,
  ...EXTRACTED_C3BC_PROJECT_FILE_SAVE_HANDLERS,
  ...
  ...EXTRACTED_D30_PROJECT_CONTAINER_HANDLERS,
...
describe("Layer 4D.R bridge handler registry", () => {
  ...
  it("extracts exactly the Wave 0, Wave 1A, Alpha3.2-C3A, Alpha3.2-C3BC, E3 media, E4 item, E5 routing/automation, D6 project tempo, D9 tracks mixer, D10 read overview/actions, D11 project marker/region, D12 transport safe, D13 items core, D14 items delete, D15 items source/phase, D16 tracks org, D17 MIDI edit, E2-FX read/write, First-Real-Fixture-A A1/A2/A3, and Safe-Write-A batches into deterministic handler modules", () => {
    ...
    assert.deepEqual(
      REGISTRY.entries
        .filter((entry) => entry.route === "d30-project-container-handlers" && entry.handler_file !== "legacy_monolith")
        .map((entry) => entry.template_id),
      [...EXTRACTED_D30_PROJECT_CONTAINER_HANDLERS.keys()],
    );
    assert.deepEqual(
      REGISTRY.entries
  ...
  it("keeps the generated bundle deterministic and registry-stamped", () => {
    ...
    assert.match(BRIDGE_SOURCE, /Handler registry: reaper\/bridge\/registry\/BRIDGE_HANDLER_REGISTRY_V1\.json \(242 registered template handler row\(s\); 0 legacy_monolith row\(s\); 242 extracted handler row\(s\); 91 handler module file\(s\)\)\./);
    ...
    assert.match(
      BRIDGE_SOURCE,
      /project\/create_cleanup_report\.lua[\s\S]*local function read_track_item_overview\(\.\.\.\)[\s\S]*OPENREAPER_HANDLER_EXPORTS\.read_track_item_overview\(\.\.\.\)/,
    );
  });

...
describe("Layer 4D.R bridge handler registry", () => {
  it("defines one standard registered handler entry shape", () => {
    ...
    assert.equal(REGISTRY.entries.length, 155);
  ...
  it("matches accepted catalog descriptors and live route allowlists exactly", () => {
    ...
    assert.deepEqual(summary, {
      ...
      extractedHandlerCount: 155,
      handlerModuleCount: 73,
      routeCount: 23,
      operationCount: 63,
    });
  ...
  it("extracts exactly the Wave 0, Wave 1A, Read-B, E3 media, E4 item, E5 routing/automation, D6 project tempo, D9 tracks mixer, D10 read overview/actions, D11 project marker/region, D12 transport safe, D13 items core, D14 items delete, D15 items source/phase, D16 tracks org, E2-FX read/write, First-Real-Fixture-A A1/A2/A3, and Safe-Write-A batches into deterministic handler modules", () => {
  ...
  it("keeps the generated bundle deterministic and registry-stamped", () => {
    ...
    assert.match(BRIDGE_SOURCE, /Handler registry: reaper\/bridge\/registry\/BRIDGE_HANDLER_REGISTRY_V1\.json \(155 registered template handler row\(s\); 0 legacy_monolith row\(s\); 155 extracted handler row\(s\); 73 handler module file\(s\)\)\./);

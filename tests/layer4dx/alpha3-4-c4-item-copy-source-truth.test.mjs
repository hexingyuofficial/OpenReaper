...
for _, count in ipairs({ 1, 8, 64 }) do
  ...
  assert(summary.rows[1].source_footprint == nil and summary.rows[1].new_item == nil)
  assert(summary.rows[1].source_item_ref == nil and summary.rows[1].target_track_ref == nil)
  assert(summary.rows[1].source_footprint_index == 1 and summary.rows[1].target_track_index == 1)
  assert(summary.rows[1].take_fx_copy.status == "passed")
  assert(summary.rows[1].take_fx_copy.source_take_ref == nil and summary.rows[1].take_fx_copy.target_take_ref == nil)
  assert(#summary.target_tracks == 1 and summary.target_tracks[1].target_track_ref == "track:guid:{TARGET}")
  assert(#summary.source_footprints == 1 and summary.source_footprints[1].source_take_ref == "take:guid:{SOURCE-TAKE}")
  assert(#refs == 0)

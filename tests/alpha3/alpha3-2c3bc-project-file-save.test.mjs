...
describe("Alpha3.2-C3B+C3C project-file save implementation", () => {
  it("adds exactly two descriptors while preserving historical and C3A count truth", () => {
    ...
    assert.equal(catalog.size, 239);
    assert.equal(CALL_TEMPLATE_RUNTIME_ACCEPTED_TEMPLATE_IDS.length, 239);
  ...
  it("uses direct APIs with fail-closed unsaved and exact post-readback verification", () => {
    ...
    assert.match(CATALOG_SUMMARY_SOURCE, /template_count = 239/);
    assert.match(CATALOG_SUMMARY_SOURCE, /project = 33/);
    assert.match(CATALOG_SUMMARY_SOURCE, /items = 36/);
    assert.match(CATALOG_SUMMARY_SOURCE, /destructive = 15/);
    assert.match(CATALOG_SUMMARY_SOURCE, /write = 129/);
    assert.match(CATALOG_SUMMARY_SOURCE, /experimental = 239/);
    assert.match(CATALOG_SUMMARY_SOURCE, /^    item = 22,$/m);
  });

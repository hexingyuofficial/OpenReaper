...
export function createAlpha345RecipeProductizationManual({ requested_ids = [] } = {}) {
  ...
  return deepFreeze({
    ...
    temporary_recipe_example: {
      ...
      copy_paste_agent_request: "Create a temporary Recipe for the current exact selected audio Items: apply this one fixed dialogue cleanup plan to all targets in one batch. Validate, save, run once, then retain evidence and delete the exact revision.",
    ...
    storage_rule: "save writes an immutable user-owned executable revision into the configured user Recipe store; list_recipes or call_recipe operation=list discovers it later. Do not ask the user to hand-edit a path or recreate the Recipe from memory.",
...
export function createAlpha345RecipeLifecycleManual() {
  ...
  const minimalDraftTemplate = {
    contract: "recipe.executable.draft.v1",
    id: "recipe.user.replace_with_stable_id",
    title: "Replace with a short user-facing title",
    summary: "Replace with the requested reusable task",
    pack: "project",
    risk: "read",
    inputs: [],
    ...
    stages: [{
      id: "run_macro",
      kind: "macro",
      dependency: { kind: "macro", id: "COPY_EXACT_MACRO_ID", version: "COPY_EXACT_VERSION", fallback_reason: null },
      inputs: [],
      outputs: ["evidence_ref"],
      risk: "COPY_EXACT_RISK",
      checkpoint: "checkpoint_run_macro",
    }],
    bindings: [{
      from: { scope: "stage", id: "run_macro", port: "evidence_ref" },
      to: { scope: "recipe_output", id: null, port: "evidence_ref" },
    }],
    dependencies: [{
      kind: "macro",
      id: "COPY_EXACT_MACRO_ID",
      version: "COPY_EXACT_VERSION",
      risk: "COPY_EXACT_RISK",
      fallback_reason: null,
      descriptor_hash: "COPY_EXACT_DESCRIPTOR_HASH",
    }],
    required_capabilities: ["COPY_EACH_EXACT_CAPABILITY"],
    risk_grants: ["COPY_EXACT_RISK"],
    checkpoints: [{
      id: "checkpoint_run_macro",
      after_stage: "run_macro",
      evidence_id: "evidence_run_macro",
      resume_identity: "resume.user.run_macro",
      summary: "Macro stage completed with retained evidence.",
    }],
    preflight: {
  ...
  return deepFreeze({
    ...
    minimal_draft_template: minimalDraftTemplate,
    request_examples: {

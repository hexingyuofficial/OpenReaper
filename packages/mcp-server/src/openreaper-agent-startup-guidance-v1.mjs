...
export const OPENREAPER_AGENT_STARTUP_GUIDANCE_SUMMARY = deepFreeze({
  ...
  caveats: {
    normal_reaper_launch_supported: true,
    only_openreaper_startup_supported: false,
    startup_lifetime_action: "openreaper-start starts REAPER when needed or attaches to the single REAPER already running from the normal icon; both paths require a matching heartbeat and bounded public read probe.",
    startup_dialog_assist: "macOS and Windows: OpenReaper observes REAPER startup windows read-only and never clicks or closes them. The normal REAPER main window and the `Scripts on`/ReaScript status window are non-blocking and must not pause startup. A blocking window returns STARTUP_USER_ACTION_REQUIRED while preserving the REAPER PID and Bridge generation.",
    bridge_start_action: `The installed conditional Scripts/__startup.lua hook starts the Bridge for normal icon launches; openreaper-start supplies session identity for agent-started launches. The REAPER action "${OPENREAPER_BRIDGE_ACTION_NAME}" is a manual recovery fallback.`,
  ...
  startup_dialog_assist: {
    ...
    consent_choices: {},
    persistent_choices: [],
    manual_behavior: "Observe all REAPER windows read-only; never click or close them. Treat the normal REAPER main window and the exact `Scripts on`/ReaScript status window as non-blocking; continue startup. Preserve the REAPER PID and Bridge generation while the user resolves a real blocker.",
    non_blocking_windows: ["REAPER main window", "Scripts on", "ReaScript run status window"],
    auto_dismisses: [],
...
export function createOpenReaperAgentStartupGuidance(input = {}) {
  ...
  const startupDialogAssist = {
    ...
    persistent_choices: [],
    policy_file: null,
    manual_behavior: "Observe REAPER windows read-only. Never click or close them. Treat the normal REAPER main window and the exact `Scripts on`/ReaScript status window as non-blocking and continue; preserve the REAPER PID and Bridge generation while the user resolves any real blocker.",
    non_blocking_windows: ["REAPER main window", "Scripts on", "ReaScript run status window"],
    auto_dismisses: [],
    auto_dismisses_with_consent: [],
    explicit_per_launch_consent: {},
  ...
  return deepFreeze({
    ...
    mode: "agent_startup_guidance",
    status: "ready",
    user_reminder: "OpenReaper MCP can attach to the single REAPER instance already running from the normal icon when its installed startup hook has started the Bridge, or start one itself. Follow docs/AGENT_START_HERE.md for the unique Agent entry, first-round Macro flow, and safety boundary.",
    agent_start_here: OPENREAPER_AGENT_START_HERE_DOCUMENT,
    first_round_flow: OPENREAPER_AGENT_FIRST_ROUND_FLOW,
    mcp_server_name: "openreaper",
    commands: {
    ...
    agent_flow: [
      startupDialogFlow,
      {
        id: "recognize_non_blocking_reaper_windows",
        when: "The startup observer reports the normal REAPER main window or an exact `Scripts on`/ReaScript status window.",
        agent_action: "Treat `Scripts on` as normal non-blocking REAPER UI. Continue waiting for the Bridge heartbeat and public read probe; do not click or close it.",
        command: null,
      },
      {
        id: "start_reaper_for_live_mcp",
        when: "The user asks for live REAPER work and no connected Bridge is available.",
        agent_action: "Run the OpenReaper startup helper. It attaches to the single REAPER already running from the normal icon when possible; otherwise it opens REAPER with the OpenReaper launch environment. It returns only after a matching heartbeat and public read probe pass, and never starts a duplicate instance.",
    ...
    requirements: {
      ...
      normal_reaper_launch_supported: true,
      only_openreaper_startup_supported: false,
      attach_existing_reaper_supported: true,

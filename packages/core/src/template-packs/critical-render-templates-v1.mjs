...
export const CRITICAL_RENDER_TEMPLATES = deepFreeze([
  ...
  },
  {
    ...
    inputSchema: objectSchema({
      ...
      format: { enum: ["wav", "ogg", "mp3", "mp4", "mov"] },
      output_policy: { const: "openreaper_managed_render_root" },
      collision_policy: { enum: ["fail_if_exists", "overwrite", "suffix"] },
      sample_rate_hz: { enum: [44100, 48000] },
      channel_count: { enum: [1, 2] },
      wav_bit_depth: { enum: [16, 24] },
      ogg_quality: { enum: [0.3, 0.5, 0.6, 0.8, 1.0] },
      mp3_bitrate_kbps: { enum: [128, 192, 256, 320] },
      video_width: { type: "integer", minimum: 16, maximum: 7680, multipleOf: 2 },
      video_height: { type: "integer", minimum: 16, maximum: 4320, multipleOf: 2 },
      video_frame_rate: { enum: [24, 25, 30, 50, 60] },
      video_codec: { const: "h264" },
      video_bitrate_kbps: { type: "integer", minimum: 256, maximum: 100000 },
      audio_codec: { const: "aac" },
      audio_bitrate_kbps: { enum: [64, 96, 128, 192, 256, 320] },
      output_basename: { type: "string", minLength: 1, maxLength: 96 },
      max_targets: { type: "integer" },
      destination: { enum: ["managed_file", "new_project_track"] },
    ...
    refs: {
      ...
      output: [
        ...
        ref("destination_track_ref", "track", false, "Created destination Track when destination is new_project_track."),
        ref("destination_item_ref", "item", false, "Created destination Item when destination is new_project_track."),
        ref("destination_take_ref", "take", false, "Created destination Take when destination is new_project_track."),
      ],
    },
    artifacts: {
      mode: "produces",
    ...
    expectedDelta: {
      ...
      entities: [
        ...
        { entity_kind: "output_file", action: "create", summary: "Managed WAV, OGG, MP3, MP4, or MOV files are created only after all expected outputs preflight cleanly." },
        ...
        { entity_kind: "project_stem", action: "create", summary: "For new_project_track, exactly one verified Track/Item/Take is created before exact source mute mutation." },
      ],
      idempotent: false,
    },
    verification: requiredVerification([
      ...
      check("output_files_verified", "file_metadata", "Every output exists, is non-empty, and has the expected audio header or ISO-BMFF container structure."),
      check("video_container_verified", "file_metadata", "MP4/MOV outputs prove ftyp, moov, mdat, H.264 video-track, dimensions, and frame-rate truth."),
      ...
      check("project_stem_verified", "state_delta", "For new_project_track, destination media truth is verified before source Tracks are muted, with exact rollback on failure."),
    ]),
    examples: [
      {
      ...
      {
        name: "render_whole_project_mp4",
        summary: "Render the whole project to managed 1080p H.264/AAC MP4 at 30 fps.",
        input: { target_kind: "whole_project", format: "mp4", output_policy: "openreaper_managed_render_root", collision_policy: "fail_if_exists", sample_rate_hz: 48000, channel_count: 2, video_width: 1920, video_height: 1080, video_frame_rate: 30, video_codec: "h264", video_bitrate_kbps: 8000, audio_codec: "aac", audio_bitrate_kbps: 192, max_targets: 1 },
      },
      {
        name: "render_selected_tracks_to_project_stem",
        summary: "Render selected Tracks to one WAV-backed project Track, verify the imported Item/Take, then mute the exact sources.",
        input: { target_kind: "selected_tracks", format: "wav", output_policy: "openreaper_managed_render_root", collision_policy: "fail_if_exists", sample_rate_hz: 48000, channel_count: 2, wav_bit_depth: 24, max_targets: 16, destination: "new_project_track", stem_mode: "mixdown", source_post_action: "mute_after_verified_insert", output_track_name: "Dialog Stem" },
      },
    ],
  },
  renderJobDescriptor({

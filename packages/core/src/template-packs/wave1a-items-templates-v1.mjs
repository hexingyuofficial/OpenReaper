...
export const WAVE1A_ITEMS_TEMPLATES = deepFreeze([
  ...
  commandDescriptor({
    ...
    summary: "Set one exact active Take or a bounded exact Item batch to reverse with one fixed native action and aggregate readback.",
    entity_kind: "take",
    tags: ["items", "take", "reverse", "batch", "native"],
    ...
    refs: refs({
      input: [ref("item_ref", "item", false, "Optional exact Item ref for the backward-compatible single-Item form; batch rows carry exact Item GUIDs internally.")],
      output: [ref("item_ref", "item", false, "Exact Item refs represented by single or aggregate readback.")],
    }),
    expectedDelta: takeUpdateDelta("Updates one or 1-64 active-Take reverse playback flags in one native batch."),
    verification: requiredVerification({
    ...
    examples: [
      ...
      {
        name: "reverse_exact_item_batch",
        summary: "Enable reverse on a bounded exact Item set in one native action.",
        input: { reverse: true, batch: [{ id: "i001", item_ref: "item:guid:{ITEM-GUID}" }], dry_run: false },
      },
    ],
  }),
  commandDescriptor({

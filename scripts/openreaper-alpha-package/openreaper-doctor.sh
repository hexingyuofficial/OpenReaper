...
console.log("important=OpenReaper can attach to a single normal-icon REAPER launch when its startup hook starts the Bridge; openreaper-start can also start or attach and verifies the live MCP session.");
...
async function smokeMcp() {
  const missing = Object.entries(report.checks)
    ...
    .map(([key]) => key);
  if (missing.length > 0) {
    await cleanupAllMcpLifecycles("doctor_required_path_missing").catch(() => {});
...
async function smokeMcpInner() {
  ...
  const packageMcpCommand = process.platform === "win32"
    ? openreaper.package_command
    : await smokeOpenReaperMcpCommandInner();
...
async function smokeOpenReaperMcpInner() {
  ...
  const liveMcpLaunch = process.platform === "win32"
    ? mcpLaunch
    : { command: process.execPath, args: [serverScript] };
  const transport = new OwnedStdioClientTransport({
    command: liveMcpLaunch.command,
    args: liveMcpLaunch.args,
    cwd: installRoot,
    env: mcpEnv,
    stderr: "pipe",
  });
    ...
    return {
      ...
      ...(process.platform === "win32" ? {
        package_command: {
          ...
          kernel: ping.kernel,
          tool_surface: toolNames,
          validation_scope: "live_installed_package_runtime",
          command: mcpCommand,
          render_root_status: ping.runtime_readiness?.render_root?.status ?? "not_observed",
        },
      } : {}),

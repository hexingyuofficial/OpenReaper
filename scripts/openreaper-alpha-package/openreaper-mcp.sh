...
for optional_openreaper_env in \
  ...
  OPENREAPER_CURRENT_PROJECT_REF; do
...
EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT="${INSTALL_ROOT}/session/executable-recipes.official"
...
if [[ -L "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}" || ( -e "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}" && ! -d "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}" ) ]]; then
  print -u2 -- "[OpenReaper] official executable recipe parent must be a non-symlink directory"
  exit 2
fi
if [[ -L "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}" || ( -e "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}" && ! -d "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}" ) ]]; then
  print -u2 -- "[OpenReaper] official executable recipe root must be a non-symlink directory"
  exit 2
fi
...
mkdir -p -- "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}"
if [[ -L "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}" || ! -d "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_PARENT}" ]]; then
  print -u2 -- "[OpenReaper] official executable recipe parent could not be safely prepared"
  exit 2
fi
mkdir -p -- "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}"
if [[ -L "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}" || ! -d "${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}" ]]; then
  print -u2 -- "[OpenReaper] official executable recipe root could not be safely prepared"
  exit 2
fi
export OPENREAPER_OFFICIAL_EXECUTABLE_RECIPE_ROOT="${EFFECTIVE_OFFICIAL_EXECUTABLE_RECIPE_ROOT}"
if [[ -z "${OPENREAPER_EXECUTABLE_RECIPE_RISK_GRANTS_JSON:-}" ]]; then
  export OPENREAPER_EXECUTABLE_RECIPE_RISK_GRANTS_JSON='["read","write","destructive"]'
fi

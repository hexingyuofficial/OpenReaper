...
`start-helper-rev=studio-hook-publish-v5`; Start refuses to spawn a dest
...
`OPENREAPER_START_HELPER_REV="studio-hook-publish-v5"`, `adopt_window_after_soft_safe`,
...
`startup-hook-poke=trusted_launcher`, `ensure_openreaper_startup_hook`,
...
trusted launcher if the status file is still missing, publish a
